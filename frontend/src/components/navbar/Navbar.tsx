  ...
  const fetchCategoryMenus = async () => {
    try {
      ...
      const filtered = categories
        ...
        .sort((a: CategoryItem, b: CategoryItem) => Number(a.sorting_nav) - Number(b.sorting_nav));
      ...
      const categoryTree = filtered
        ...
        .map((parent: CategoryItem) => ({
          ...parent,
          children: filtered.filter((child: CategoryItem) => child.id_parent === parent.id_kategori)
        }));
...
const renderMenus = (menus: (MenuItem | CategoryItem)[], depth = 0, isMobile = false) => {
  ...
  return menus.map((menu) => {
    ...
    if (currentMenuType === 'main') {
      ...
      return (
        <div key={menuItem.id_menu} className={`navbar-menu-item position-relative ${isMobile ? "w-100" : "d-inline-block"} ${isMobile ? "py-2 border-bottom border-gray-200" : ""}`}>
          ...
          {isMobile ? (
            ...
            <>
              <div className="d-flex justify-content-between align-items-center px-3 py-2 cursor-pointer"
                ...
                <a href={menuItem.menu_url || "#"} className="text-blue-900 d-block text-decoration-none flex-grow-1"
                  onClick={(e) => { if (hasChildren) e.preventDefault(); }}>
                  {menuItem.menu_name}
                </a>
                {hasChildren && <ChevronDown size={16} className={`transition-transform ${isSubOpen ? "rotate-180" : ""}`} />}
              </div>
              {hasChildren && isSubOpen && (
                <div className="ms-4 mt-2">
                  {menuItem.children!.map((child) => (
                    <a key={child.id_menu} href={child.menu_url || "#"} className="d-block px-3 py-2 text-blue-900 hover-bg-blue-50 rounded text-decoration-none"
            ...
            <div className="position-relative d-inline-block h-100">
              <a href={menuItem.menu_url || "#"} className={`px-3 py-2 d-inline-block transition-colors duration-300 navbar-menu-link h-100 d-flex align-items-center ${depth === 0 ? "text-white hover-text-decoration-underline" : "hover-bg-blue-100 text-blue-900"}`}>
                {menuItem.menu_name}
              ...
              {hasChildren && depth === 0 && (
                <div className="submenu-hidden">
                  {menuItem.children!.map((child) => (
                    <a key={child.id_menu} href={child.menu_url || "#"} className="d-block px-4 py-2 text-blue-900 hover-bg-blue-50 text-decoration-none">
                      {child.menu_name}
                    </a>
                  ))}
                </div>
      ...
      const hasChildren = categoryItem.children && categoryItem.children.length > 0;
      const isSubOpen = openSubmenu === String(categoryItem.id_kategori);

      return (
        <div key={categoryItem.id_kategori} className={`navbar-menu-item position-relative ${isMobile ? "w-100" : "d-inline-block"} ${isMobile ? "py-2 border-bottom border-gray-200" : ""}`}>
          ...
          {isMobile ? (
            ...
            <>
              <div className="d-flex justify-content-between align-items-center px-3 py-2 cursor-pointer"
                ...
                <a href={`/berita/kategori/${categoryItem.slug}`} className="text-blue-900 d-block text-decoration-none flex-grow-1"
                  onClick={(e) => { if (hasChildren) e.preventDefault(); }}>
              ...
              {hasChildren && isSubOpen && (
                <div className="ms-4 mt-2">
                  {categoryItem.children!.map((child) => (
                    <a key={child.id_kategori} href={`/berita/kategori/${child.slug}`} className="d-block px-3 py-2 text-blue-900 hover-bg-blue-50 rounded text-decoration-none"
                      onClick={() => setIsMenuOpen(false)}>
                      {child.kategori}
                    </a>
                  ))}
            ...
            <div className="position-relative d-inline-block h-100">
              <a href={`/berita/kategori/${categoryItem.slug}`} className={`px-3 py-2 d-inline-block transition-colors duration-300 navbar-menu-link h-100 d-flex align-items-center ${depth === 0 ? "text-white hover-text-decoration-underline" : "hover-bg-blue-100 text-blue-900"}`}>
                {categoryItem.kategori}
                {hasChildren && <ChevronDown size={14} className="ms-1" />}
              </a>
              {hasChildren && depth === 0 && (
                <div className="submenu-hidden">
                  {categoryItem.children!.map((child) => (
                    <a key={child.id_kategori} href={`/berita/kategori/${child.slug}`} className="d-block px-4 py-2 text-blue-900 hover-bg-blue-50 text-decoration-none">
                      {child.kategori}
                    </a>
                  ))}
                </div>
  ...
  return (
    <nav className="bg-gradient-to-r-blue-800-950 text-white rounded-top-6 rounded-top-sm-4 rounded-top-md-4 rounded-top-lg-4 rounded-top-xl-4 rounded-top-xxl-4 shadow-md">
      ...
      <div className="d-flex align-items-center position-relative px-4 px-md-5 px-lg-6 px-xl-6 px-xxl-6 py-6 h-20 flex-wrap">
        ...
        <div className="d-none d-md-flex gap-3 position-absolute top-6 start-6">
        ...
        {/* Bahasa - Desktop Only */}
        <div className="d-none d-md-flex gap-2 position-absolute top-6 end-6 z-index-50">
          <img
            src="/assets/indo.png"
        ...
        {/* Logo & Judul */}
        <div className="flex-grow-1 d-flex flex-column flex-md-column align-items-center text-center mt-5 z-10">
          <img
            src="/assets/logo.png"
            alt="Logo Kominfo"
          ...
          <button
            className="d-md-none text-white hamburger-btn position-absolute"
...
<div className="w-100 navbar-wrapper d-none d-md-block">
  <div className="d-flex justify-content-between align-items-center min-width-max px-3 px-md-5 py-3 font-semibold text-md" style={{ maxWidth: '4000px', margin: '0 auto' }}>
